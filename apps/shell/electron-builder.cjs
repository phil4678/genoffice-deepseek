...
// artifact. The downstream layout (feed archive name, DeepOffice-intel.dmg
...
const config = {
  appId: 'com.deepoffice.app',
  productName: 'DeepOffice',
  ...
  mac: {
    ...
    // electron-builder's default arch-less names (DeepOffice-<v>.dmg /
    // DeepOffice-<v>-mac.zip). Both zips land in one latest-mac.yml and
  ...
  linux: {
    ...
    // DeepOffice-<v>.AppImage / genoffice_<v>_amd64.deb — because the public
    ...
    // StartupWMClass falls back to productName ("DeepOffice"), which does not
  ...
  // "DeepOffice" and only happens to downcase it to the right value.
  deb: {
