...
function makeAccess(opts?: {
  ...
  const access: DeckAccess = {
    ...
    isCloudPageGenEnabled: async () => !opts?.localPages,
    ...
    generatePageLocal: opts?.localPages
      ? async (args) => {
...
describe('generate_deck self-driven page-by-page generation', () => {
  it('local path (no cloud): pages generated by generatePageLocal html → all land via generateFromHtml', async () => {
    const { access, localPageCalls, getPages, landOrder } = makeAccess({ localPages: true })
    const skill = createSlidesSkill(access)

    const res = (await skill.executeTool(deckCall(3))) as { output: string; summary: string }

    // The tool internally calls generatePageLocal once per page and lands the html strings
    expect(localPageCalls.sort((a, b) => a - b)).toEqual([1, 2, 3])
    expect(getPages()).toBe(3)
    expect(landOrder[0]).toMatch(/^replace:.*PAGE1:/)
